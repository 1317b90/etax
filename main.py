...
def main(data:models.Shell):
    try:
        ...
        make_invoice.main(models.Execute(**data.dict()))
        ...
        if task:
            ...
            try:
                ...
                if data.get("is_group",False):
                    data["group_name"] = user_data["GroupName"]

                # 补充开票习惯
                try:
                    invoiceHabit=json.loads(user_data["InvoiceHabit"])
                ...
                if result is not None:
                    ...
                    tell.done_task(data.taskid,False,"多次重试后失败",result_sum)
                    tell.clear_memory(data.userid)

            except Exception as e:
                try:
                    tell.send_message(data.serviceid,data.userid,data.adminid,"出现意外错误："+str(e))
                    tell.done_task(data.taskid,False,"出错",str(e))
                    tell.clear_memory(data.userid)
                    traceback.print_exc()
                except Exception as e:
                    traceback.print_exc()
